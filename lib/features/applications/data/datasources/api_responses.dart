...
class ApiResponse {
  ...
  callApplicationList(String? token) async {
    var response = await ApiServices().applicationList(token);
    if (response.statusCode == 200) {
      applicationController.applications.value =
          Applications.fromJson(response.body);
      await applicationController.box
          .write(LocalDatabase().applications, response.body);
